...
if strcmp(model.diagonalFinalLikelihoodNoise, 'yes') 
   ...
   for i=1:model.N  
    num_OfNonNans(i) = sum(~isnan(model.Y(i,:)));  
   end
else
...
%
   ...
   %     
        ...
        for t=1:model.T 
            ...
            if length(ind) > 0
               ...
               if strcmp(model.diagonalFinalLikelihoodNoise, 'yes')
                 ...
                 %rrt = model.Y(ind,t)./(sqrt(model.sigma2(ind))') - LLobs*model.Ft(:,t);
                 %gradFt = LLobs'*rrt;           
               else
      ...
      if strcmp(model.diagonalFinalLikelihoodNoise, 'yes')
        ...
        beta_post = model.priorSigma2.beta0 + 0.5*nansum( (model.Y - (model.L.*model.Weights)*model.Ft).^2, 2)'; 
   ...
   %    
    ...
    %
        ...
        if strcmp(model.useFactorModel, 'yes')
           if strcmp(model.diagonalFinalLikelihoodNoise, 'yes')
           tmp3 = bsxfun(@times, 1./model.sigma2', (model.Y - (model.L.*model.Weights)*model.Ft).^2); 
